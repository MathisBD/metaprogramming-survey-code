...
/-- `MyFunctor` is similar to `Functor` except that it does not have a `mapConst` method. -/
...
-- For some reason Lean does not pick up the Functor instance we just defined.
